...
\begin{DoxyCode}
79     \textcolor{keyword}{def }\hyperlink{namespacedialog__babi__feedback_1_1build_a7a9d289f7493a5ded13c4b7f071b6184}{build}(cls, dpath):
80         print(\textcolor{stringliteral}{"building version: %s"} % cls.version)
81 
82         \textcolor{comment}{# the root of dataset}
83         dpext = os.path.join(dpath, \textcolor{stringliteral}{'insuranceQA-master/%s'} % cls.version)
84 
85         \textcolor{comment}{# read vocab file}
86         vocab\_path = os.path.join(dpext, \textcolor{stringliteral}{"vocabulary"})
87         d\_vocab = cls.read\_vocab(vocab\_path)
88 
89         \textcolor{comment}{# read label2answer file}
90         label2answer\_path\_gz = os.path.join(dpext, cls.label2answer\_fname)
91         d\_label\_answer = cls.read\_label2answer(label2answer\_path\_gz, d\_vocab)
92 
93         \textcolor{comment}{# Create out path}
94         out\_path = os.path.join(dpath, cls.version)
95         build\_data.make\_dir(out\_path)
96 
97         \textcolor{comment}{# Parse and write data files}
98         cls.write\_data\_files(dpext, out\_path, d\_vocab, d\_label\_answer)
99 
100 
\end{DoxyCode}
...
\begin{DoxyCode}
53     \textcolor{keyword}{def }read\_label2answer(cls, label2answer\_path\_gz, d\_vocab):
54         lines = cls.readlines(label2answer\_path\_gz)
55 
56         d\_label\_answer = \{\}
57         \textcolor{keywordflow}{for} line \textcolor{keywordflow}{in} lines:
58             fields = line.rstrip(\textcolor{stringliteral}{"\(\backslash\)n"}).split(\textcolor{stringliteral}{"\(\backslash\)t"})
59             \textcolor{keywordflow}{if} len(fields) != 2:
60                 \textcolor{keywordflow}{raise} ValueError(
61                     \textcolor{stringliteral}{"label2answer file (%s) corrupted. Line (%s)"}
62                     % (repr(line), label2answer\_path\_gz)
63                 )
64             \textcolor{keywordflow}{else}:
65                 aid, s\_wids = fields
66                 sent = cls.wids2sent(s\_wids.split(), d\_vocab)
67                 d\_label\_answer[aid] = sent
68         \textcolor{keywordflow}{return} d\_label\_answer
69 
\end{DoxyCode}
...
\begin{DoxyCode}
37     \textcolor{keyword}{def }read\_vocab(cls, vocab\_path):
38         d\_vocab = \{\}
39         with open(vocab\_path, \textcolor{stringliteral}{"r") as f:}
40 \textcolor{stringliteral}{            }\textcolor{keywordflow}{for} line \textcolor{keywordflow}{in} f:
41                 fields = line.rstrip(\textcolor{stringliteral}{'\(\backslash\)n'}).split(\textcolor{stringliteral}{"\(\backslash\)t"})
42                 \textcolor{keywordflow}{if} len(fields) != 2:
43                     \textcolor{keywordflow}{raise} ValueError(
44                         \textcolor{stringliteral}{"vocab file (%s) corrupted. Line (%s)"}
45                         % (repr(line), vocab\_path)
46                     )
47                 \textcolor{keywordflow}{else}:
48                     wid, word = fields
49                     d\_vocab[wid] = word
50         \textcolor{keywordflow}{return} d\_vocab
51 
\end{DoxyCode}
...
\begin{DoxyCode}
25     \textcolor{keyword}{def }readlines(cls, path):
26         \textcolor{keywordflow}{if} path.endswith(\textcolor{stringliteral}{".gz"}):
27             lines = cls.read\_gz(path)
28         \textcolor{keywordflow}{else}:
29             lines = open(path).readlines()
30         \textcolor{keywordflow}{return} lines
31 
\end{DoxyCode}
